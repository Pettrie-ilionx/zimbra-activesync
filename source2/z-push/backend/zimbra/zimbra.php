<?php
$GLOBALS['revision'] = "66"; // Used to output the script version to the debug log
/***********************************************
* File          :   zimbra.php
* Revision      :   66 (10-Jan-2017)
...
*                   Julien Laurent
*
* Changes       :   Changes Made To Revision 66: z-push-2 version ONLY
    ...
    public function Logon($username, $domain, $password) {
        ...
        if (file_exists($multiFolderSupportFile)) {
            ...
            if (defined('ZIMBRA_DEBUG')) {
                if ((ZIMBRA_DEBUG === true) || (ZIMBRA_DEBUG == 'setup')  || (stripos(ZIMBRA_DEBUG, $username) !== false)) {
...
//        $this->_password = $password;    // Fix to replace symbols in user passwords.
        ...
        if (!$response) {
            if (isset($this->_soapError) && (($this->_soapError == 'CURL.7') || (substr($this->_soapError, 0, 4) == 'HTML'))) {
                    ...
                    if (ZIMBRA_RETRIES_ON_HOST_CONNECT_ERROR !== false) {
                        ...
                        if ($retries > 0) {
                            for ($i=0;$i<$retries;$i++) {
                                ...
                                $response = $this->SoapRequest($body, $header, true, $returnJSON);
                                if ($response) break;
                                if (($this->_soapError != 'CURL.7') && (substr($this->_soapError, 0, 4) != 'HTML')) break;
                            }
                        }
                    }
                }
...
//                ZLog::Write(LOGLEVEL_DEBUG, 'Zimbra->Logon(): ' .  'AuthResponse: '. print_r( $contents, true ), false );
...
//                ZLog::Write(LOGLEVEL_DEBUG, 'Zimbra->Logon(): ' .  'AuthResponse: contents: '. print_r( $contents, true ), false );
...
//                    ZLog::Write(LOGLEVEL_DEBUG,  "NoOpResponse: ".print_r( $contents, true) , false );
            ...
            /* Nokia E72 and other similar Nokia Messaging phones need to think the server can
               sync the Outbox folder in order to be able to send emails. 
               If the Outbox does not appear in the Folder Hierarchy from the server, the phone 
               just leaves outgoing emails sitting in the Outbox folder on the phone.
                   
               This has been found to be needed for android 2.3.3 and later - so removing Nokia check
...
*/
    ...
    public function Setup($store, $checkACLonly = false, $folderid = false, $readonly = false) {
        ...
        if ($store != false) {
            ...
            */
            ...
            if (defined('ZIMBRA_DEBUG')) {
                if ((ZIMBRA_DEBUG === true) || (ZIMBRA_DEBUG == 'setup')  || (stripos(ZIMBRA_DEBUG, $this->_username) !== false)) {
    ...
    function GetZimbraSmartFolders( &$user, &$contents) {
        ...
        if (defined('ZIMBRA_DEBUG')) {
            $debugSetup = ((ZIMBRA_DEBUG === true) || (ZIMBRA_DEBUG == 'setup') || (stripos(ZIMBRA_DEBUG, $this->_username) !== false));
                    ...
                    foreach ($this->_contactMapping as $k => $v) {
                        if ($k <> '' && $v <> '') {
                            ...
                            if(isset($param[$v]) && ($k == 'birthday' || $k == 'anniversary') ) {
                                    ...
                                    // To Do - Validate if this can be further limited
                                    if (($k == 'birthday') &&
                                        ((stripos($this->_ua, "MailforExchange") === false) && (stripos($this->_ua, "Apple") === false))) {
                                        // Do nothing to avoid constant resync attempts
                                        ZLog::Write(LOGLEVEL_DEBUG, 'Zimbra->GetMessage(): ' .  "UserAgent [".$this->_ua."] - Not Apple or Nokia - SKIP BIRTHDAY" );
                                    } else {
...
//                                $subApp->sensitivity = 0;
                            ...
                            if($cAttendees > 0) {
                                ...
                                for($j=0; $j<$cAttendees; $j++) {
                                    ...
                                    // Only output attendee status to organizer
                                    if ($isOrganizer) {
...
//        $soap = '<SendInviteReplyRequest xmlns="urn:zimbraMail" id="'.$requestid.'" compNum="0" verb="'.$verb.'" updateOrganizer="'.$updateOrganizer.'"/>';
        ...
        if($response) {
            ...
            unset($response);
            $calendarid = '';
//			debugLog( 'INVITE-RESPONSE:' . print_r( $array, true), false );
...
//    protected function SoapRequest($body,$header=false,$connecting=false) {
    function SoapRequest($body,$header=false,$connecting=false,$returnJSON=false) {
        ...
        elseif (($returnJSON == true) && (strpos($response,'"Body":{"Fault":')!==false)) {
            ...
            $this->error = 'SOAP FAULT: Error Code Returned ['.$this->_soapError.'] ';
            ZLog::Write(LOGLEVEL_ERROR, 'Zimbra->SoapRequest(): ' . $this->error);
            $response = false;
...
//    private $connection;
     ...
     */
    public function BackendSearchZimbra($backend) {
...
//                ZLog::Write(LOGLEVEL_DEBUG, 'Zimbra->GetDocumentLibrarySearchResults(): ' . print_r($row,true));
    ...
    public function GetGALSearchResults($searchquery, $searchrange) {
        ZLog::Write(LOGLEVEL_DEBUG, 'Zimbra->GetGALSearchResults(): ' .  'START GetGALSearchResults { searchquery = ' . $searchquery . '; searchrange = ' . $searchrange . ' }');
